...
%       pinfo.clabels_short : shortform text labels for communities
...
%% Community names: parcellation specific
C                       = []; 
    ...
    if exist(lutfile,'file')
        ...
        if ~isempty(S)
            ...
            if ~isempty(max(sub_rois))
                rois                        = rois + max(sub_rois);         % adjust cortical rois to account for subcrotex
            end
        ...
        %% Create masks for this parcellation (Only works for Schaefer?)
        if ~strncmpi(shortname,'scha',4)                                    % IF NOT using one of the schaefer parcellations
            ...
            disp(' See end lines of conn_GetParcInfo.m')
%             keyboard
        end
        ...
        Ncl             = length(pinfo(pp).cis);                            % Number of communities
        Clbls           = pinfo(pp).clabels;                                % community labels
        ...
        % resting-state networks within each & between all
        if ~strncmpi(shortname,'scha',4)
            maskn=[];
        else
            CIs             = pinfo(pp).cis;
            ...
            dt              = dt+dt.';
            dt(dt==0)       = nan;
            dt(dt==2)       = 0;                                            % between network
            for xx=1:Ncl; dt(CIs{xx},CIs{xx}) = xx; end                     % within network
            maskn           = dt;
            maskn(maskut)   = nan;
        end
        ...
        pinfo(pp).masks.rsnetwork   = maskn;                                % 0 = between, 1:7 = rs-networks (Clbls)
        clear tmp*
